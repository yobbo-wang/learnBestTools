...
Xcodebuild() {
    ...
    if $is_workspace ; then
        ...
        # 编译前做clear操作
        xcodebuild clean -workspace $project_dir/ios/${project_name}.xcworkspace \
        -scheme ${scheme_name} \
        -configuration ${build_configuration}
        # archive操作
        xcodebuild archive -workspace $project_dir/ios/${project_name}.xcworkspace \
        -scheme ${scheme_name} \
        -configuration ${build_configuration} \
        -archivePath ${export_archive_path}
    else
        xcodebuild clean -project $project_dir/ios/${project_name}.xcodeproj \
        -scheme ${scheme_name} \
        -configuration ${build_configuration}

        xcodebuild archive -project $project_dir/ios/${project_name}.xcodeproj \
        -scheme ${scheme_name} \
        -configuration ${build_configuration} \
        -archivePath ${export_archive_path}
    fi
